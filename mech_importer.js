...
/* to get stats for new frames, run frames.json through this:
frameStats = {};
...
function parse(ccCode, type) {
    if (!(type in ccCodeDict && type in sheetData)) console.warn("Warning: Couldn't find type \"" + type + "\" for \"" + ccCode + "\"");
...
printMech = function printMech(pilot) {
...
// character sheet for this mech needs to be opened, with the callsign as its name
    enterMacros = function enterMacros(pilot) {
    let callsign = pilot.callsign.replace(/″/g,"");
    let activeMech = !pilot.mechs.map(mech => mech.id).includes(pilot.state.active_mech_id) ? pilot.mechs[0] : pilot.mechs.find(mech => {
        return mech.id === pilot.state.active_mech_id
    });
    console.log("active mech: ", JSON.stringify(activeMech));
    ...
    if (!r20MechSheet) {
        console.warn("Could not find character sheet for callsign \"" + callsign + "\". Make sure that the character sheet is open and has the correct name.");
        return
    }
    if (r20MechSheet.parentNode.attributes.style.textContent.includes("display: none")) {
        console.warn("Please exit edit mode for callsign \"" + callsign + "\".");
        return
    }
...
document.getElementById("journal").getElementsByClassName("superadd")[0].insertAdjacentHTML( 'afterend', "<div style=\"float: inline-end;\"><button class=\"btn\" float=\"inline\" id=\"importbutton\">Import</button><input type=\"text\" id=\"importtext\" placeholder=\"Paste pilot data here\"></div>");
document.getElementById("importbutton").addEventListener("click", () => enterMacros(JSON.parse(document.getElementById("importtext").value)))
